...
cntj (57)"""
*/
    val input = """mypyemv (1058) -> tdssotr, pebnvks, zaulju
snslv (48)
aytbgf (30)
mkcrsy (42)
shwcd (42)
...
tnmtz (26)"""
    ...
    val tree = Day7a.solveFor(input.split('\n'))

    println(tree.first)

    println(Day7b.solveFor(tree.first, tree.second))
}

data class Node(val name: String, val weight: Int, val subtree: List<String> = emptyList(), var totalWeight: Int = 0)

object Day7a {

    private val structure = mutableMapOf<String, Node>()

    fun solveFor(split: List<String>): Pair<String, MutableMap<String, Node>> {

        split.forEach {
            val parts = it.split(" -> ")
            val name = parts[0].split(' ')[0]
            val weight = parts[0].split('(')[1].substringBefore(')').toInt()
            if (parts.size > 1) {
                val subnodes = parts[1].split(", ")
                structure[name] = Node(name, weight, subnodes)
            } else {
                structure[name] = Node(name, weight)
            }
        }

        val nodes = structure.mapValues { 0 }.toMutableMap()
        structure.forEach { _, node ->
            node.subtree.forEach {
                nodes[it] = 1
            }
        }

        return nodes.filterValues { it == 0 }.keys.toList()[0] to structure
    }
...
object Day7b {
    ...
    fun solveFor(bottomProgram: String, structure: MutableMap<String, Node>): Int {
        weight(bottomProgram, structure)
        return findNeededWeight(bottomProgram, structure, -1)
    }

    private fun weight(program: String, structure: MutableMap<String, Node>): Int {
        val node = structure[program]!!
        val totalWeight = node.weight + node.subtree.sumBy { weight(it, structure) }
        node.totalWeight = totalWeight
        return totalWeight
    }

    private fun findNeededWeight(program: String, structure: Map<String, Node>, targetWeight: Int): Int {

        val node = structure[program]!!
        val subnodes: List<Node> = node.subtree.map { structure[it]!! }
        val different: Node? = getDifferentNode(subnodes)

        if (different == null) {
            return targetWeight - (node.totalWeight - node.weight)
        } else {
            return findNeededWeight(different.name, structure, subnodes.filter { it != different }[0].totalWeight)
        }
    }

    private fun getDifferentNode(nodes: List<Node>): Node? {
        val offWeight: Int? = nodes.groupBy { it.totalWeight }.mapValues { it.value.size }.filterValues { it == 1 }.keys.toList().lastOrNull()
        return nodes.firstOrNull { it.totalWeight == (offWeight ?: 0) }
    }
