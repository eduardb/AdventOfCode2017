...
e inc -223 if ku >= 3325"""
    println(Day8a.solveFor(input.split('\n')))
}


object Day8a {
    fun solveFor(instructions: List<String>): Pair<Int, Int> {
        ...
        instructions.forEach {
            ...
            val instruction = Instruction(
                    register = tokens[0],
                    operation = Op.from(tokens[1]),
                    value = tokens[2].toInt(),
                    conditionRegister = tokens[4],
                    condition = Comparator.from(tokens[5]),
                    conditionValue = tokens[6].toInt()
            )

            if (instruction.condition(registers[instruction.conditionRegister] ?: 0, instruction.conditionValue)) {
                registers[instruction.register] = instruction.operation(registers[instruction.register] ?: 0, instruction.value)
                if (registers[instruction.register]!! > highestEver) {
                    highestEver = registers[instruction.register]!!
                }
            }

        }

        return registers.values.max()!! to highestEver
    }
...
sealed class Op {
    ...
    companion object {
        ...
        fun from(op: String) = when (op) {
            "inc" -> Inc
            "dec" -> Dec
            else -> throw IllegalArgumentException("$op not valid")
        }
    }
    ...
    abstract operator fun invoke(left: Int, right: Int): Int
}

sealed class Comparator {

    companion object {
        fun from(comparator: String) = when (comparator) {
            "<" -> LT
            "<=" -> LTE
            ">" -> GT
            ">=" -> GTE
            "==" -> EQ
            "!=" -> NEQ
            else -> throw IllegalArgumentException("$comparator not valid")
        }
    }

    object LT : Comparator() {
        override fun invoke(left: Int, right: Int) = left < right
    }

    object LTE : Comparator() {
        override fun invoke(left: Int, right: Int) = left <= right
    }

    object GT : Comparator() {
        override fun invoke(left: Int, right: Int) = left > right
    }

    object GTE : Comparator() {
        override fun invoke(left: Int, right: Int) = left >= right
    }

    object EQ : Comparator() {
        override fun invoke(left: Int, right: Int) = left == right
    }

    object NEQ : Comparator() {
        override fun invoke(left: Int, right: Int) = left != right
    }

    abstract operator fun invoke(left: Int, right: Int): Boolean
}
