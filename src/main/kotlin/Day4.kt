...
//        |oiii ioii iioi iiio
//    """.trimMargin()
    val input = """pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
...
juo pmiyoh xxk myphio ogfyf dovlmwm moevao qqxidn"""
    println(Day4a.solveFor(input))
    println(Day4b.solveFor(input))
}

object Day4a {
    fun solveFor(input: String): Int = input.split('\n').sumBy {
        if (isValid(it)) 1 else 0
    }

    private fun isValid(passphrase: String): Boolean {
        val words = mutableMapOf<String, Int>()
        passphrase.split(' ').forEach {
            words[it] = 1 + (words[it] ?: 0)
        }
        words.forEach {
            if (it.value > 1)
                return false
...
object Day4b {
    ...
    private fun sort(word: String): String = word.toCharArray().sortedArray().contentToString()
